...
static inline int getexp( unsigned int size )
{
	...
	{
		#ifdef DEBUG
		fprintf(stderr, "getexp returns -1 for %i less than MINEXP\n", size );
		#endif
	...
	#ifdef DEBUG
	fprintf(stderr, "getexp returns %i (%i bytes) for %i size\n", shift - 1, (1<<(shift -1)), size );
	#endif
...
static void dump_array()
{
	...
	fprintf(stderr, "------ Free pages array ---------\n");
	fprintf(stderr, "System memory allocated: %i\n", l_allocated );
	fprintf(stderr, "Memory in used (malloc'ed): %i\n", l_inuse );

		for ( i = 0; i < MAXEXP; i++ )
		{
			fprintf(stderr, "%.2i(%i): ",i, l_completePages[i] );
	
			tag = l_freePages[ i ];
			while ( tag != NULL )
			{
				if ( tag->split_left  != NULL  ) fprintf(stderr, "*");
				fprintf(stderr, "%i", tag->real_size );
				if ( tag->split_right != NULL  ) fprintf(stderr, "*");
	...
	fprintf(stderr, "'*' denotes a split to the left/right of a tag\n");
	// fflush( stdout );
}
#endif
...
static struct boundary_tag* allocate_new_tag( unsigned int size )
{
	...
	struct boundary_tag *tag;
		...
		#ifdef DEBUG
		fprintf(stderr, "Resource allocated %p of %i pages (%i bytes) for %i size.\n", tag, pages, pages * l_pageSize, size );

		l_allocated += pages * l_pageSize;

		fprintf(stderr, "Total memory usage = %i KB\n",  (int)((l_allocated / (1024))) );
		#endif
...
void *malloc(size_t size)
{
	...
	liballoc_lock();
		...
		#ifdef DEBUG
		fprintf(stderr, "Found tag with %li bytes available (requested %li bytes, leaving %li), which has exponent: %i (%i bytes)\n", tag->real_size - sizeof(struct boundary_tag), size, tag->real_size - size - sizeof(struct boundary_tag), index, 1<<index );
		#endif
		...
		if ( ((int)(remainder) > 0) /*&& ( (tag->real_size - remainder) >= (1<<MINEXP))*/ )
		{
			...
			{
				#ifdef DEBUG
				fprintf(stderr, "Seems to be splittable: %i >= 2^%i .. %i\n", remainder, childIndex, (1<<childIndex) );
				#endif
				...
				#ifdef DEBUG
				fprintf(stderr, "Old tag has become %i bytes, new tag is now %i bytes (%i exp)\n", tag->real_size, new_tag->real_size, new_tag->index );
				#endif
			}	
		}
...
void free(void *ptr)
{
	...
	liballoc_lock();
		...
		{
			#ifdef DEBUG
			fprintf(stderr, "Melting tag right into available memory. This was was %i, becomes %i (%i)\n", tag->real_size, tag->split_right->real_size + tag->real_size, tag->split_right->real_size );
			#endif
		...
		{	
			...
			if ( l_completePages[ index ] == MAXCOMPLETE )
			{
				...
				#ifdef DEBUG
				l_allocated -= pages * l_pageSize;
				fprintf(stderr, "Resource freeing %p of %i pages\n", tag, pages );
				dump_array();
				#endif
	...
	#ifdef DEBUG
	fprintf(stderr, "Returning tag with %i bytes (requested %i bytes), which has exponent: %i\n", tag->real_size, tag->size, index ); 
	dump_array();
	#endif
