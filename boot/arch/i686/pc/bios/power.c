...
void _pc_poweroff()
{
    ...
    if (uacpi_unlikely_error(uacpi_status)) {
        panic(uacpi_status, "failed to prepare for sleep: %s", uacpi_status_to_string(uacpi_status));
    }
    ...
    if (uacpi_unlikely_error(uacpi_status)) {
        panic(uacpi_status, "failed to enter sleep: %s", uacpi_status_to_string(uacpi_status));
    }
