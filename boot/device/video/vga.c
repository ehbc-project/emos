...
static status_t set_mode_vbe(struct device *dev, int mode)
{
    ...
    if (vbe_mode_info.memory_model == VBEMM_TEXT) {
        ...
        mm_map(vbe_mode_info.framebuffer, (void *)vbe_mode_info.framebuffer, ALIGN((vbe_mode_info.lin_num_image_pages + 1) * hw_frame_size, 4096) >> 12, PF_WTCACHE);
...
static status_t get_mode_info_vga(struct device *dev, int mode, struct video_mode_info *mode_info)
{
    ...
    switch (mode) {
        ...
        case 0x03:
            ...
            mode_info->next_mode = 0x0C;
            mode_info->text = 1;
            mode_info->width = 80;
            mode_info->height = 25;
            break;
        case 0x0C:
            mode_info->current_mode = 0x0C;
...
static status_t get_hw_mode_info_vbe(struct device *dev, int mode, struct video_hw_mode_info *hwmode)
{
    ...
    if (!data->mode_set_by_vbe) return STATUS_CONFLICTING_STATE;
