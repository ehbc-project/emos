...
static void console_putchar(struct device *dev, char ch)
{
    struct ansiterm_data *data = (struct ansiterm_data *)dev->data;
...
static void put_char(struct device *dev, char ch)
{
    struct ansiterm_data *data = (struct ansiterm_data *)dev->data;
    ...
    switch (ch) {
        ...
        case '\033':
            ...
            break;
        case 0x7F:
            if (console_backspace(dev, &cursor_x)) break;
...
static int probe(struct device *dev)
{
    ...
    struct device *condev = dev->reference->dev;
    if (!condev) return 1;
